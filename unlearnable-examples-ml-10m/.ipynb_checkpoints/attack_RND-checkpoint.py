...
Path50 = "./preprocessed/sample50-15.npy"

def train_attack():
    print('----------------training processing------------------')
    # Load preprocess data
    with open('./preprocessed/instant_video.pickle', 'rb') as f:
...
def evaluate(PATH):
    ...
    with open('./preprocessed/instant_video.pickle', 'rb') as f:
        ...
        val_user_list = dataset['val_user_list']
    ...
    val_set_len = len(val_pair)
    train_dataset = TripletUniformPair(item_size, new_fake_train_user_list, ng_num, train_set_len)
    train_loader = DataLoader(train_dataset, batch_size=batch_size, shuffle=True)
    val_dataset = TripletUniformPair(item_size, val_user_list, ng_num, val_set_len)
    val_loader = DataLoader(val_dataset, batch_size=batch_size, shuffle=False)
    val_loader.dataset.ng_sample()
    ...
    for epoch in range(max_epoch):
        ...
        model.eval()
        with torch.no_grad():
            for idx, (u, i, j) in enumerate(val_loader):
                u, i, j = u.cuda(), i.cuda(), j.cuda()
                val_loss = model(u, i, j)
