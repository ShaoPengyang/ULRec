...
max_added = attacked_user_size * 14
# -5 -10 -15 -2022
...
def train_attack():
    ...
    init part
    '''
    with open('./preprocessed/gowalla.pickle', 'rb') as f:
    ...
    if attacked_user_size == 50:
        attacked_users = np.load("./preprocessed/sample50-2022.npy", allow_pickle=True)
        ...
        print("Please enter attacked user size number: 50/100/500")
        import sys
        sys.exit()
    ...
    print('K is:'+str(K))
    '''
    K=100 is not work! Why? Worse than Adv
    '''
    ...
    while True:
        ...
        for attacked_user in attacked_users:
            ...
            # record_data_tensor_u[init_top_2K_item[attacked_user]] = 1.0
            '''
            sample goal: no training set & no top ranking items. 
            '''
            set4sample = torch.where(record_data_tensor_u == 0)[0]
            # sampled_idx = np.random.choice(set4sample.shape[0], size=1000, replace=False)
        ...
        if result == True or len(added_items) > max_added:
            save_path = "./preprocessed/our"+ str(attacked_user_size) +".npy"
            print(save_path)
...
if __name__ == '__main__':
    if choice == 0:
        ...
        PATH_MODEL = './preprocessed/our' + str(attacked_user_size) + '.npy'
        print(PATH_MODEL)
        evaluate(PATH_MODEL)
    elif choice == 2:
        PATH_MODEL = './preprocessed/our' + str(attacked_user_size) + '.npy'
